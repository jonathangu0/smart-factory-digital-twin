...
- Rotation pivot is the turntable axis at **(0.046, −0.173)** (the base discs
  `NAUO2811` / `NAUO2861`).
- The arm was offset ~0.108 m in X from the column in the raw CAD; the driver applies
...
- **Static** (correctly *not* moved): the base/turntable, the two vacuum cylinders
  ...
  (`NAUO2862`). An early bug rotated these — fixed.
...
Oven + turntable + saw + conveyor. The signature motion is the **turntable**.
...
- The driver shows the sort clearly: while `act_SLD` is on, a workpiece travels the
  belt and is ejected sideways into the bin matching the **live colour flag**
  (`sld_white` / `sld_red` / `sld_blue`). Three colour bin markers show the targets.

Live source: `act_SLD` + the three colour counters.
