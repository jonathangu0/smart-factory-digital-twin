"""Compute the real factory bounds from meshes, add ground + an overview camera,
re-save, and screenshot."""
import os
import omni.usd
from pxr import Usd, UsdGeom, Gf, Vt

SAVE = r"C:/Users/icets/Downloads/digitaltwinsf/TrainingFactory_Industry40.usd"
SHOT = r"C:/Users/icets/Downloads/digitaltwinsf/twin/shots/factory_02.png"
...
def main():
    ...
    # real bounds = union of mesh world bounds (root bbox returns a placeholder)
    ...
    d = max(dims)
    print("meshes:", n, "real bbox", [round(v, 2) for v in mn], [round(v, 2) for v in mx])
    print("center", [round(v, 2) for v in C], "dims", [round(v, 2) for v in dims])

    # ground at the model's floor
    ...
    gx.AddScaleOp().Set(Gf.Vec3f(max(dims[0], dims[1]) * 1.4, max(dims[0], dims[1]) * 1.4, 0.01))
    g.GetDisplayColorAttr().Set(Vt.Vec3fArray([Gf.Vec3f(0.22, 0.23, 0.25)]))

    # overview camera at an elevated 3/4 angle
    cam = UsdGeom.Camera.Define(stage, "/World/OverviewCam")
    eye = C + Gf.Vec3d(d * 1.1, -d * 1.2, d * 0.85)
    ...
    # make the overview camera active + screenshot
